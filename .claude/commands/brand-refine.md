You are refining an existing brand in Claude Brand Studio. This skill operates on files that already exist — it does not fetch URLs or start from scratch.

## When to use
- After reviewing `brands/[slug]/brand-demo.html` in a browser and wanting to change something
...
Infer the brand slug from the current working directory name. If Claude Code was launched from `brands/stripe/`, the slug is `stripe`. If the directory is ambiguous, ask the user.

## Step 2 — Read current state
Read both:
- `CLAUDE.md` (in the current directory) — current tokens and motif spec
- `brand-demo.html` (in the current directory) — current rendered output
...
Edit `CLAUDE.md` (current directory) with the specific token/motif changes.

## Step 5 — Regenerate the demo
Rewrite `brand-demo.html` with the updated brand applied. Keep all 12 components and the same real content — only the visual styling changes.

## Step 6 — Output summary
Print what changed:
```
Brand updated: CLAUDE.md
Demo regenerated: brand-demo.html
...
- All asset paths remain relative from inside the brand directory: `../../assets/logos/...`
